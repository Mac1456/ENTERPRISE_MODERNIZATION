...
/**
 * Migration script for Real Estate Contact Management Features
 * Run this script to set up all database tables and features for Feature 3
 */

require_once __DIR__ . '/../config/database.php';

class RealEstateMigration
{
    private $db;
    ...
    public function run()
    {
        echo "Starting Real Estate Contact Management Migration...\n";
        
        try {
            $this->createContactsCustomTable();
            $this->createPropertiesTable();
            $this->createPropertyInterestsTable();
            $this->createPropertyShowingsTable();
            $this->createSavedSearchesTable();
            $this->createPropertyAlertsTable();
            $this->createPropertyFeaturesReference();
    ...
    private function createContactsCustomTable()
    {
        echo "Creating contacts_cstm table...\n";
        
        $sql = "CREATE TABLE IF NOT EXISTS contacts_cstm (
            ...
            INDEX idx_contact_type (contact_type_c),
            INDEX idx_timeline (timeline_c),
            INDEX idx_urgency (urgency_level_c),
            INDEX idx_budget (budget_min_c, budget_max_c),
            INDEX idx_lead_score (lead_quality_score_c)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci";
        
        if (!$this->db->query($sql)) {
            throw new Exception('Failed to create contacts_cstm table: ' . $this->db->error);
        }
    }
    ...
    private function createPropertiesTable()
    {
        ...
        if (!$this->db->query($sql)) {
            throw new Exception('Failed to create properties table: ' . $this->db->error);
        }
    }
    
    private function createPropertyInterestsTable()
    {
        echo "Creating contact_property_interests table...\n";
        
        $sql = "CREATE TABLE IF NOT EXISTS contact_property_interests (
            id CHAR(36) NOT NULL PRIMARY KEY,
        ...
        if (!$this->db->query($sql)) {
            throw new Exception('Failed to create contact_property_interests table: ' . $this->db->error);
        }
    }
    
    private function createPropertyShowingsTable()
    {
        echo "Creating property_showings table...\n";
        
        $sql = "CREATE TABLE IF NOT EXISTS property_showings (
            id CHAR(36) NOT NULL PRIMARY KEY,
            contact_id CHAR(36) NOT NULL,
            property_id CHAR(36) NOT NULL,
        ...
        if (!$this->db->query($sql)) {
            throw new Exception('Failed to create property_showings table: ' . $this->db->error);
        }
    }
    
    private function createSavedSearchesTable()
    {
        echo "Creating contact_saved_searches table...\n";
        
        $sql = "CREATE TABLE IF NOT EXISTS contact_saved_searches (
            id CHAR(36) NOT NULL PRIMARY KEY,
            contact_id CHAR(36) NOT NULL,
        ...
        if (!$this->db->query($sql)) {
            throw new Exception('Failed to create contact_saved_searches table: ' . $this->db->error);
        }
    }
    
    private function createPropertyAlertsTable()
    {
        echo "Creating property_alerts table...\n";
        
        $sql = "CREATE TABLE IF NOT EXISTS property_alerts (
            id CHAR(36) NOT NULL PRIMARY KEY,
            contact_id CHAR(36) NOT NULL,
            property_id CHAR(36) NOT NULL,
        ...
        if (!$this->db->query($sql)) {
            throw new Exception('Failed to create property_alerts table: ' . $this->db->error);
        }
    }
    
    private function createPropertyFeaturesReference()
    {
        echo "Creating property_features_reference table...\n";
        
        $sql = "CREATE TABLE IF NOT EXISTS property_features_reference (
        ...
        if (!$this->db->query($sql)) {
            throw new Exception('Failed to create property_features_reference table: ' . $this->db->error);
        }
        ...
        $stmt = $this->db->prepare("INSERT IGNORE INTO property_features_reference (category, feature_name, description) VALUES (?, ?, ?)");
        foreach ($features as $feature) {
            $stmt->bind_param("sss", $feature[0], $feature[1], $feature[2]);
            $stmt->execute();
        }
    }
    ...
    private function insertSampleData()
    {
        ...
        $sampleContactId = $this->generateUUID();
        $samplePropertyId = $this->generateUUID();
        $sampleInterestId = $this->generateUUID();
        
        // Insert sample contact custom data
        $sql = "INSERT IGNORE INTO contacts_cstm (
            id_c, contact_type_c, budget_min_c, budget_max_c, budget_approved_c,
            timeline_c, urgency_level_c, lead_quality_score_c, engagement_level_c,
            property_type_preferences_c, location_preferences_c
        ) VALUES (
            '{$sampleContactId}', 'buyer', 300000, 500000, 1,
            '3-6months', 'high', 85, 'high',
            '[\"Single Family Home\", \"Condo\"]', '[\"Downtown\", \"Suburbs\"]'
        )";
        $this->db->query($sql);
        
        // Insert sample property
        $sql = "INSERT IGNORE INTO properties (
            id, name, description, property_type, status, street_address, city, state,
            bedrooms, bathrooms, square_footage, list_price, features
        ) VALUES (
            '{$samplePropertyId}', 'Beautiful Family Home', 'Spacious 4-bedroom home in great neighborhood',
            'single_family', 'active', '123 Main Street', 'Anytown', 'CA',
            4, 3.5, 2500, 450000, '[\"Hardwood Floors\", \"Updated Kitchen\", \"Garage\"]'
        )";
        $this->db->query($sql);
        
        // Insert sample property interest
        $sql = "INSERT IGNORE INTO contact_property_interests (
            id, contact_id, property_id, interest_level, notes
        ) VALUES (
            '{$sampleInterestId}', '{$sampleContactId}', '{$samplePropertyId}', 'very_interested',
            'Perfect size for our family, love the location'
        )";
        $this->db->query($sql);
...
if (php_sapi_name() === 'cli') {
    try {
        $migration = new RealEstateMigration();
        $migration->run();
    } catch (Exception $e) {
